...
The DAG goes through a sequence of changes once all predicate trees are known.

1. All predicate trees are combined to create the initial DAG, merging any common subtrees.
...
4. If step 3 changes the DAG, go to step 2.
...
DAG Evaluation is the process by which the values of nodes in the DAG are determined.  When a node associated with a rule because truthy, that rule is fired.  The DAG is evaluated on a per-transaction basis.
...
We have made it this far without actually worrying about what the value return by a function is.  As an example of how values can be complex, consider the following expressions:
    ...
    -- (gt 5 (transformation 'length' (sub 'a' (var 'ARGS'))))
...
All drawings are done via [GraphViz][] dot format.
