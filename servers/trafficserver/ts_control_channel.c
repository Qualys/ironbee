...
/**
 * @file
 * @brief IronBee --- Apache Traffic Server Control Channel Plugin
 *
 * @author Nick LeRoy <nleroy@qualys.com>
 */

/**
 * @defgroup IronBeeTrafficControlChannelPlugin
 * IronBee Traffic Server Control Channel plugin
...
struct module_data_t {
    ...
    const char                          *socket_path;     /**< Path to socket */
...
)
{
    ...
    optind = 1;    /* Reset */
    while (c = getopt(argc, (char**)argv, "s:m:"), c != -1) {
        switch(c) {
        case 'm':
            mod_data->ts_module_path = ib_mm_strdup(mod_data->mm, optarg);
            break;
        case 's':
            mod_data->socket_path = ib_mm_strdup(mod_data->mm, optarg);
            break;
        default:
 ...
 *
 * @param[in] sig Signal number (should be SIGIO)
 */
static void sigio_handler(int sig)
{
    assert(sig == SIGIO);
    assert(sigio_data.mod_data != NULL);
    assert(sigio_data.mod_data->channel != NULL);

    ib_status_t    rc = IB_OK;

    /* Receive message */
    rc = ib_engine_manager_control_recv(sigio_data.mod_data->channel);
    if (rc != IB_OK) {
        TSError("[ironbee] Failed to read from the control channel: %s",
                ib_status_to_string(rc));
    }
}

/**
 * Create and start the control channel, make it async, enable SIGIO
 *
 * @param[in] mod_data Module data
 *
 * @returns Status code
 */
static ib_status_t control_channel_setup(
    module_data_t *mod_data
)
{
    assert(mod_data != NULL);
    ...
    rc = ib_engine_manager_control_channel_create(&channel,
                                                  mod_data->mm,
                                                  mod_data->ts_module_data->manager);
    if (rc != IB_OK) {
        TSError("[ironbee] Failed to create control channel: %s", ib_status_to_string(rc));
        return rc;
    }

    /* Set the path */
    rc = ib_engine_manager_control_channel_socket_path_set(channel, mod_data->socket_path);
    if (rc != IB_OK) {
        TSError("[ironbee] Failed to set control channel socket path: %s",
                ib_status_to_string(rc));
        return rc;
    }

    /* Register commands */
    rc = ib_engine_manager_control_manager_ctrl_register(channel);
    if (rc != IB_OK) {
        TSError("[ironbee] Failed to register control channel comamnds: %s",
                ib_status_to_string(rc));
        return rc;
    }

    /* Register echo */
    rc = ib_engine_manager_control_echo_register(channel);
    if (rc != IB_OK) {
        TSError("[ironbee] Failed to register control channel echo command: %s",
                ib_status_to_string(rc));
        return rc;
    }

    /* Start the control channel */
    rc = ib_engine_manager_control_channel_start(channel, &sock);
    if (rc != IB_OK) {
        TSError("[ironbee] Failed to start control channel: %s", ib_status_to_string(rc));
        return rc;
    }
    ...
    sysrc = fcntl(sock, F_SETFL, O_NONBLOCK | O_ASYNC | FASYNC);
    if (sysrc < 0) {
        TSError("[ironbee] Failed to make control socket asynch: %s",
                strerror(errno));
        return IB_EOTHER;
    }

    /* Allow the process to receive SIGIO */
    sysrc = fcntl(sock, F_SETOWN, getpid());
    if (sysrc < 0) {
        TSError("[ironbee] Failed to change ownership of control socket: %s",
                strerror(errno));
        return IB_EOTHER;
    }

    /* Done */
    mod_data->channel = channel;
...
/**
 * Initialize the ATS IronBee Control Channel plugin.
 *
...
void TSPluginInit(int argc, const char *argv[])
{
    ...
    registration_info.plugin_name = (char *)"ironbee control channel";
    ...
    rc = ib_mpool_create(&mpool, "ATS Control Channel Plugin", NULL);
    if (rc != IB_OK) {
    ...
    rc = control_channel_setup(mod_data);
    if (rc != IB_OK) {
        goto done;
    }

    TSDebug("ironbee", "Control channel plugin initialized");
