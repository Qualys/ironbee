...
static const size_t MIN_BS_FLAGBUF = 32;    /**< Min byte string flag buffer */
 ...
 * @param[in] buf Buffer to format (must be at least min_bs_flagbuf)
 *
 * @returns @a buf
 */
static const char *format_flagbuf(
    ib_status_t  rc,
    ib_flags_t   flags,
    char        *buf
)
{
    ...
    /* If there's an error, ignore everything else */
    if (rc != IB_OK) {
        snprintf(buf, MIN_BS_FLAGBUF, " [%s]", ib_status_to_string(rc));
        return buf;
    }
 ...
 * @param[in] quote Quoting mode
 * @param[in] maxlen Maximum string length (0 = none, clipped to MAX_BS_LEN)
 * @param[in] flagbuf Buffer for flags (must be NULL or size >= MIN_BS_FLAGBUF)
...
)
{
    ...
    *pescaped = buf;
    ...
    if (flagbuf != NULL) {
        format_flagbuf(rc, flags, flagbuf);
    }
    return rc;
}
...
)
{
    ...
    for (node = header->head; node != NULL; node = node->next) {
        const char *nbuf;
        char        nfbuf[MIN_BS_FLAGBUF];
        const char *vbuf;
        char        vfbuf[MIN_BS_FLAGBUF];
...
)
{
    ...
    IB_LIST_LOOP_CONST(lst, node) {
        ...
        case IB_FTYPE_BYTESTR:
            txdump_field(tx, txdump, nspaces, escaped, field, 0);
            break;
